export default function StepCard({ number, title, time, status, children }) {
  ...
  return (
    <div className={`border rounded-md mb-4 transition-all ${
      isActive
      ...
      <div className={`flex items-center gap-3 px-4 py-3 border-b ${
        ...
        <span className={`text-sm font-semibold flex-1 ${isComplete ? 'text-[#3fb950]' : 'text-[#e6edf3]'}`}>
          {title}
        </span>
        {time && (
      ...
      {/* Body */}
      <div className={`px-4 py-4 ${isComplete && status === 'locked' ? 'opacity-60 pointer-events-none' : ''}`}>
        {children}
      </div>
    </div>
  );
